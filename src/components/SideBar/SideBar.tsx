...
export default function SideBar() {
  ...
  return (
    <aside className={`transition-all duration-300 ${isCollapsed ? "w-[80px]" : "w-[280px]"
      } h-[100vh] bg-neutral-85 pb-6`}>
      <nav className="h-full flex flex-col justify-between">
        <div>
          <div className={`${isCollapsed ? "relative" : ""} flex justify-between items-center p-7`}>
            <Image width={`${isCollapsed ? "40" : "157"}`} height={40} src={`${isCollapsed ? "/logos/logo-light.svg" : "/logos/logo-text-light.svg"}`} alt="로고" />
            <div className={`${isCollapsed ? "w-8 h-8 p-2 absolute top-16 -right-4 bg-neutral-80 rounded-full" : ""}`}>
              <Image width={24} height={24} src={`${isCollapsed ? "/icons/direction/bar-right.svg" : "/icons/direction/bar-left.svg"}`} alt="화살표" className={`${isCollapsed ? "filter invert" : "filter invert"}`} onClick={toggleSidebar} />
            </div>
          </div>
