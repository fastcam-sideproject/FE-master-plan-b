    ...
    <>
      <div className='flex gap-20'>
        <div className='flex flex-col gap-20'>
          <OnboardingSubject
            title={<>현재 종사하거나 관심있는 직무를 선택해주세요</>}
            p={'선택한 분야에 맞게 자격증을 추천해드릴게요!'}
          />
          <div className='flex gap-20'>
            ...
            <div className='flex flex-col gap-4'>
              {jobTags.map(tag => (
                <Chip key={tag} size={'large'} keyword={tag} onClick={() => handleRemoveTag(tag)} />
              ))}
            </div>
          </div>
          <JobDropdownToggle onSelect={(main, sub) => handleAddTag(`${main} / ${sub}`)} />
        </div>
      </div>
